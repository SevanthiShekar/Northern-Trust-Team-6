...
// retry
app.post("/payments/:id/retry", async (req, res) => {
    try {
...
// Debug: Fetch all raw payments (bypassing schema)
app.get("/payments-raw", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ 
            success: false,
            message: "Error fetching raw payments",
            error: err.message 
        });
    }
});

// Fetch payment summary with aggregation
